...
def predict(input_ids, attention_mask): # Updated function signature
...
for _ in range(100):  # Number of iterations
    _ = predict(inputs.input_ids, inputs.attention_mask) # Pass input_ids and attention_mask separately
end_time = time.time()
...
start_time = time.time()
for _ in range(100):  # Number of iterations
    _ = predict_jit(inputs.input_ids, inputs.attention_mask) # Pass input_ids and attention_mask separately
end_time = time.time()
print(f"Average inference time (with JIT): {(end_time - start_time) / 100:.4f} seconds")

# Benchmark batched inference
batch_start_time = time.time()
_ = predict_jit(inputs.input_ids, inputs.attention_mask)  # Single batched inference
batch_end_time = time.time()
print(f"Batched inference time: {batch_end_time - batch_start_time:.4f} seconds for {len(texts)} samples")
